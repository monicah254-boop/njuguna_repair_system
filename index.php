<?php
// 1. Import central cloud database handler (Manages Render vs Local automatically)
require_once 'db.php';
...
// Handle Tracking Lookup via Phone Number
if (isset($_GET['customer_phone'])) {
    $search_phone = trim($_GET['customer_phone']);
    if (!empty($search_phone)) {
        $stmt = $pdo->prepare("SELECT job_id, customer_name, device_model, problem_description, status, created_at FROM job_cards WHERE customer_phone = ? OR customer_phone LIKE ? ORDER BY created_at DESC");
        $stmt->execute([$search_phone, "%$search_phone%"]);
        $jobs = $stmt->fetchAll();
        
        if (empty($jobs)) { $error = "No active repair records found for phone number: $search_phone"; }
    } else { $error = "Please enter your registered phone number."; }
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .hero-section { background: linear-gradient(135deg, #0d6efd 0%, #0043a8 100%); color: white; padding: 40px 20px; border-radius: 0 0 20px 20px; }
    ...
    <div class="card p-4 text-start mb-4">
        <form action="index.php" method="GET">
            ...
            <div class="input-group">
                ...
                <input type="text" class="form-control form-control-lg" name="customer_phone" placeholder="e.g. 0712345678" value="<?php echo htmlspecialchars($search_phone); ?>" required>
        ...
        ?>
            <div class="card p-4 text-start mb-4 shadow-sm">
                <div class="d-flex justify-content-between align-items-center mb-3 border-bottom pb-2">
                    <h5 class="fw-bold text-dark mb-0"><i class="bi bi-phone"></i> <?php echo htmlspecialchars($job['device_model']); ?></h5>
                ...
                <div class="position-relative mb-5 mt-4 mx-2">
                    ...
                    <div class="position-absolute top-50 translate-middle text-center" style="left: 0%;">
                        <span class="badge rounded-circle p-2 <?php echo $step >= 1 ? 'bg-success' : 'bg-secondary'; ?>"><i class="bi bi-file-earmark-medical"></i></span>
                    ...
                    <div class="position-absolute top-50 translate-middle text-center" style="left: 50%;">
                        <span class="badge rounded-circle p-2 <?php echo $step >= 2 ? 'bg-success' : 'bg-secondary'; ?>"><i class="bi bi-tools"></i></span>
                    ...
                    <div class="position-absolute top-50 translate-middle text-center" style="left: 100%;">
                        <span class="badge rounded-circle p-2 <?php echo $step == 3 ? 'bg-success' : 'bg-secondary'; ?>"><i class="bi bi-check-circle"></i></span>
                ...
                <?php if ($status != 'completed'): ?>
                    <div class="bg-light p-3 rounded-3 border border-warning-subtle mt-2">
                        ...
                        <p class="text-muted mb-2" style="font-size: 12px;">Need something else changed while your phone is disassembled? Type it below to update the technician instantly.</p>
                        ...
                        <form action="index.php" method="POST">
                            ...
                            <div class="input-group">
                                <input type="text" name="additional_request" class="form-control form-control-sm" placeholder="e.g. Please also replace the battery / screen protector..." required>
                                <button type="submit" name="submit_request" class="btn btn-success btn-sm fw-bold">Update Request</button>
                            </div>
                        </form>
                    </div>
                <?php else: ?>
                    <div class="alert alert-success py-2 small mb-0 mt-2"><i class="bi bi-bag-check-fill"></i> Repair Complete! Please visit our shop collection point.</div>
                <?php endif; ?>
            </div>
        <?php endforeach; ?>
    <?php endif; ?>
