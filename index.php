...
if (isset($_GET['customer_phone'])) {
    ...
    if (!empty($search_phone)) {
        try {
            $stmt = $pdo->prepare("SELECT job_id, customer_name, device_model, problem_description, status, created_at FROM job_cards WHERE customer_phone = ? OR customer_phone LIKE ?");
...
<div class="hero-section text-center shadow-sm mb-4">
    <div class="container" style="max-width: 650px;">
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <a href="login.php" class="btn btn-light btn-sm fw-semibold text-primary px-3"><i class="bi bi-bi-shield-lock"></i> Staff Login</a>
        </div>
        ...
        <?php foreach ($jobs as $job): ?>
            ...
            <div class="card p-4 text-start mb-4 shadow-sm">
                <div class="d-flex justify-content-between align-items-center border-bottom pb-2">
                    <span class="badge bg-secondary">Job #<?php echo echo $job['job_id']; ?></span>
                    <span class="text-muted small"><?php echo $job['device_model']; ?></span>
                </div>
